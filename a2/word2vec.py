...
def naiveSoftmaxLossAndGradient(
    ...
    probability = lambda Ux: math.e**(np.dot(Ux, centerWordVec)) / sum(math.e**np.dot(outsideVectors, centerWordVec))
    loss = -np.log(probability(Uo))
    y_vec = probability(outsideVectors)
    gradCenterVec = -Uo + np.sum(np.multiply(y_vec, outsideVectors.T),axis = 1)
    gradOutsideVecs = centerWordVec * y_vec[:, np.newaxis]
    gradOutsideVecs[outsideWordIdx] = np.multiply(centerWordVec, (y_vec[outsideWordIdx] - 1).T)
...
def negSamplingLossAndGradient(
    ...
    ### END YOUR CODE
    Uo = outsideVectors[outsideWordIdx]
    negWords = outsideVectors[negSampleWordIndices]
    loss = -np.log(sigmoid(np.dot(Uo, centerWordVec))) - np.sum(np.log(sigmoid(-np.dot(negWords, centerWordVec))))
    sig_vec = 1 - sigmoid(-np.dot(negWords, centerWordVec))
    gradCenterVec = -np.outer(Uo, (1 - sigmoid(np.dot(Uo, centerWordVec)))).flatten() + np.sum([n*s for s,n in zip(sig_vec,negWords)], axis = 0).T
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    negGrad = centerWordVec*np.array([sum(sig_vec[sig_vec == s]) for s in sig_vec ])[:, np.newaxis]
    for i, x in zip(negSampleWordIndices, negGrad):
        gradOutsideVecs[i] = x
    gradOutsideVecs[outsideWordIdx] = -centerWordVec * (1 - sigmoid(np.dot(Uo, centerWordVec)))
    return loss, gradCenterVec, gradOutsideVecs
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    ### END YOUR CODE
    outsideVectorsSample = [word2Ind[word] for word in outsideWords]
    centerWordVector = centerWordVectors[word2Ind[currentCenterWord]]
    losses = 0
    for idx in outsideVectorsSample:
        loss, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(centerWordVector, idx, outsideVectors, dataset)
        losses += loss
        gradCenterVecs[word2Ind[currentCenterWord]] += gradCenterVec
        gradOutsideVectors += gradOutsideVecs
    return losses, gradCenterVecs, gradOutsideVectors
