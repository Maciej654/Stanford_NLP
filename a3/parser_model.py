...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        xx = torch.zeros(( embeddings.shape[1] * n_features, hidden_size))
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(xx))
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.zeros(hidden_size)))
        self.dropout = nn.Dropout(dropout_prob)
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(torch.zeros((hidden_size, n_classes))))
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform_((torch.zeros(n_classes))))
    ...
    def forward(self, w):
        ...
        z = (x @ self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        relu = nn.ReLU()
        h = relu(z)
        h = self.dropout(h)
