...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens = pad_packed_sequence(enc_hiddens)[0].permute(1, 0, 2)  # (b, src_len, 2*h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        Ybar_t = torch.unsqueeze(Ybar_t, 0)
        dec_hidden = torch.unsqueeze(dec_state[0], 0)
        dec_cell = torch.unsqueeze(dec_state[1], 0)
        O_t, dec_state = self.decoder(Ybar_t, (dec_hidden, dec_cell))
        (dec_hidden, dec_cell) = dec_state
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.permute(1, 2, 0))
        ...
        alpha_t = softmax(e_t)
        alpha_t = torch.unsqueeze(alpha_t, dim=0)
        a_t = torch.bmm(enc_hiddens.permute(0, 2, 1), alpha_t.permute(1, 2, 0)).permute(2,1,0)
        a_t = torch.squeeze(a_t, 0)
        U_t = torch.cat((torch.squeeze(dec_hidden, dim = 0).T, a_t), dim=0).T
        ...
        combined_output = O_t
        dec_hidden = torch.squeeze(dec_state[0], 0)
        dec_cell = torch.squeeze(dec_state[1], 0)
